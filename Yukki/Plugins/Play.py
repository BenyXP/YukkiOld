...
async def music_onoff(_, message: Message):
    ...
    if status in ("ON", "on", "On"):
        ...
        if not message.chat.id in DISABLED_GROUPS:
            return await lel.edit("» **Music Telah aktif✅**")
        ...
        if message.chat.id in DISABLED_GROUPS:
            return await lel.edit("» **Music Tidak Aktif Tolol❌**")
        DISABLED_GROUPS.append(message.chat.id)
        await lel.edit(f"✅ **Musicnya Dimatiin Di {message.chat.title}**\n\n• Mangkanya Jangan Musikan terus ngentot...")
    else:
        return await message.reply_text(
            "😕 **Ngetik yang bener Tolol.**\n\n» Coba `/music on` atau `/music off`"
        )
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        try:
            ...
            pass
        if audio.file_size > 99999999999:
